...
class Game {
    ...
    constructor(world: WorldData, duration: number) {
    ...
    displayRobotStats(robotID: number) {
        ...
        if(this.robotData[robotID].isDisplayed) {
            ...
            topTextFrame += lineSpacing * 0.5;
            topTextFrame = this.paper.showStatus(centerTextFrame, topTextFrame, 'Net Worth', this.robotData[robotID].stats.worth, statRGB, false);
            ...
            for(var i = 0; i < defender.items.length; i ++) {
                if(defender.items[i].isEquipped) {
                    ...
                    if(damage[j] > 0) {
                        ...
                        while(defender.items.length > 0) this.dropItem(defender, 0);
    ...
    dropItem(robot: RobotData, itemID: number) {
