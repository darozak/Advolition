...
Each item has a bulk value. The more bulk that a robot is carrying, the longer it will take to complete an action.
...
# Winning the Game
The robot begins the game on its home tile. When time runs out, robots will be scored based on the total worth of items located on their home tile to include the robot (assuming it made it back in time) and any items that it is carying.
