...
class RampedArray {
     ...
     */
    constructor(restingArray, activatedArray, pulsePattern) {
        this.restingArray = restingArray;
        this.activatedArray = activatedArray;
        this.pulsePattern = pulsePattern;
        while (this.pulsePattern.length < 3)
            this.pulsePattern.push(0);
        ...
        while (this.restingArray.length < this.activatedArray.length)
            this.restingArray.push(0);
        while (this.activatedArray.length < this.restingArray.length)
            this.activatedArray.push(0);
    }
    /**
    ...
    value() {
        ...
        if (this.currentFrame < this.endPoint[0]) {
            ...
            if (this.hold)
                this.currentFrame = this.endPoint[1] - 1;
        }
